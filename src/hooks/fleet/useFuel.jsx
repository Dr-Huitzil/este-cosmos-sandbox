...
export function useFuel({ user, firestore, toast, selectedVehicleId, isFull, isAiAuthorized, handleCloseFuelLog }) {
  ...
  const handleMigrateMPG = useCallback(async () => {
    ...
    try {
      const allEntries = [...fuelEntries].sort((a, b) => (a.odometer || 0) - (b.odometer || 0)); // Oldest first for processing
      const descendingEntries = [...fuelEntries].sort((a, b) => (b.odometer || 0) - (a.odometer || 0)); // Newest first for calculateMPG
      const batch = writeBatch(firestore);
      let writes = 0;
      for (const entry of allEntries) {
        const newMPG = calculateMPG(entry, descendingEntries);
