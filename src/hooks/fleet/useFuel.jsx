...
export function useFuel({ user, firestore, toast, selectedVehicleId, isFull, isAiAuthorized, handleCloseFuelLog }) {
  ...
  const handleMigrateMPG = useCallback(async () => {
    ...
    try {
      // For migrate, we need to process all entries.
      // calculateMPG now requires a pre-sorted array newest-first.
      // We can use the existing sortedFuelEntries or recreate it.
      const dateSortedEntries = [...fuelEntries].sort((a, b) => {
        const d1 = new Date(b.day).getTime();
        const d2 = new Date(a.day).getTime();
        if (d1 !== d2) return d1 - d2;
        return (b.odometer || 0) - (a.odometer || 0);
      ...
      let writes = 0;
      for (const entry of dateSortedEntries) {
