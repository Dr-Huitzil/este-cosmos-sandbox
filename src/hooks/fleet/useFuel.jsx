...
export function useFuel({ user, firestore, toast, selectedVehicleId, isFull, isAiAuthorized, handleCloseFuelLog }) {
  ...
  const handleAddFuelLog = useCallback(
    async (e) => {
      ...
      // ⚡ BOLT OPTIMIZATION: Pass sorted arrays to calculateMPG
  ...
  const handleMigrateMPG = useCallback(async () => {
    ...
    try {
      ...
      // ⚡ BOLT OPTIMIZATION: Sort entries newest-first by date (and odometer desc as tie-breaker) for calculateMPG which expects descending order
      const allEntriesDesc = [...fuelEntries].sort((a, b) => {
        const d1 = new Date(b.day).getTime();
        const d2 = new Date(a.day).getTime();
        if (d1 !== d2) return d1 - d2;
        return (b.odometer || 0) - (a.odometer || 0);
      ...
      for (const entry of allEntries) {
        const newMPG = calculateMPG(entry, allEntriesDesc);
