...
export function useFuel({ user, firestore, toast, selectedVehicleId, isFull, isAiAuthorized, handleCloseFuelLog }) {
  ...
  const handleMigrateMPG = useCallback(async () => {
    ...
    try {
      // The original calibration sorted ascending (oldest-first) for chronological iteration:
      const allEntries = [...fuelEntries].sort((a, b) => (a.odometer || 0) - (b.odometer || 0));
      // For calculateMPG which expects newest-first, we use the already pre-sorted context array:
      const newestFirstEntries = sortedFuelEntriesRef.current;

      const batch = writeBatch(firestore);
      let writes = 0;
      for (const entry of allEntries) {
        const newMPG = calculateMPG(entry, newestFirstEntries);
