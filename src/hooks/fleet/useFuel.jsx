...
export function useFuel({ user, firestore, toast, selectedVehicleId, isFull, isAiAuthorized, handleCloseFuelLog }) {
  ...
  const handleMigrateMPG = useCallback(async () => {
    ...
    try {
      // Create a separate sorted array for iteration (oldest first if desired, or just use fuelEntries)
      // but pass the newest-first sortedFuelEntries to calculateMPG.
