...
export function useFuel({ user, firestore, toast, selectedVehicleId, isFull, isAiAuthorized, handleCloseFuelLog }) {
  ...
  const handleMigrateMPG = useCallback(async () => {
    ...
    try {
      // Create a sorted list for iteration, but note calculateMPG expects newest-first order
      const entriesToUpdate = [...fuelEntries].sort((a, b) => (a.odometer || 0) - (b.odometer || 0));
