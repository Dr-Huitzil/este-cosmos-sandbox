...
export function useFuel({ user, firestore, toast, selectedVehicleId, isFull, isAiAuthorized, handleCloseFuelLog }) {
  ...
  const handleMigrateMPG = useCallback(async () => {
    ...
    try {
      // Create a chronological sort for processing, but also need newest-first for calculateMPG
      const allEntriesChronological = [...fuelEntries].sort((a, b) => (a.odometer || 0) - (b.odometer || 0));
      // sortedFuelEntries is already newest-first
