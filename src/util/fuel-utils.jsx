 ...
 *
 * IMPORTANT: sortedEntries must be pre-sorted newest-first by date before calling.
 * Pre-sorting once at the call site avoids redundant O(N log N) sorts on every render,
 * preventing O(N^2 log N) performance degradation when called inside a loop.
 *
 * @param {{ odometer: number, fuelQuantity: number, isFull: boolean}} currentEntry
 * @param {Array<{ odometer: number, fuelQuantity: number, isFull: boolean, day: string}>} sortedEntries
 * @returns {number}
 */
