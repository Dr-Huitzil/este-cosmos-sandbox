 ...
 * pre-sorting once at the call site (useMemo) avoids redundant O(N log N) sorts
 *
 * @param {{ odometer: number, fuelQuantity: number, isFull: boolean}} currentEntry
 * @param {Array<{ odometer: number, fuelQuantity: number, isFull: boolean, day: string}>} allEntries (sorted newest-first)
 * @returns {numbers}
 */
...
export function calculateMPG(currentEntry, allEntries) {
  ...
  // ⚡ Bolt: Removed internal array sort. Expects allEntries to be pre-sorted newest-first
  // This prevents O(N^2 log N) performance degradation when called inside a render loop
